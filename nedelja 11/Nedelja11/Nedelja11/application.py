...
class StudentGradeApp(ctk.CTk):  
    ...
    def update_grade(self):
        if self.selected_grade_id is not None and self.grade.get():
            try:
                ...
                self.refresh_data()
                messagebox.showinfo("Uspjesno","Ocjena je promjenjena")
                self.grade.set("")
                self.selected_grade_id=None
            except Exception as e:
                messagebox.showerror("Greska",(str(e)))
        else:
            messagebox.showwarning("Izaber ocjenu","Klikni dva puta na polje")
    ...
    def filter_grades(self):
        ...
        course_kw=self.search_student_var.get().lower()
        for i in self.tv.get_children():
            self.tv.delete(i)
        for row in self.db.fetch_grades():
            student_name=row[1].lower()
            course_name=row[2].lower()
