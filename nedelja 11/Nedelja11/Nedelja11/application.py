...
class StudentGradeApp(ctk.CTk):  
    ...
    def create_widgets(self):  
        ...
        ctk.CTkButton(tab_students, text="Add Student", command=self.add_students).grid(row=2, column=1, columnspan=2, pady=10)  
        ...
        ctk.CTkButton(tab_grades,text="Filter grades",command="").grid(row=4,column=0,columnspan=2,padx=10,pady=5)
        ctk.CTkButton(tab_grades, text="Update grade",command="").grid(row=5,column=0,pady=5)
        ctk.CTkButton(tab_grades, text="Selete grade",command="").grid(row=5,column=1,pady=5)

        self.tv=ttk.Treeview(tab_grades,columns=(1,2,3,4,5,6),show="headings",height=10)
        self.tv.grid(row=6,column=0,columnspan=2,sticky="nsew")
        ...
        self.tv.bind("<Double-1>") #dodati komandu odje za izbor ocjene
